...
parser.add_argument('--easy_task', type=str, help='If set, will exclude occluded and parked cars from training')
...
MODEL = 'rcnn'  # or 'yolo'
if parser.parse_args().easy_task.lower() in ['true', '1', 'yes']:
    EASY_TASK = True
else:
    EASY_TASK = False
...
def main():
    # 1. Load Data
    full_dataset = AICityDataset(video_path=VIDEO_PATH, xml_path=XML_PATH, easy_task=EASY_TASK)
