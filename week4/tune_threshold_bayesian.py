...
def main():
    ...
    # ---------------------------------------------------------
    def optimize_mtmc(thresh):
        print(f"\n[Testing Threshold: {thresh:.4f}]")
        ...
        # The optimizer will attempt to maximize this return value
        return idf1

    # ---------------------------------------------------------
    # RUN OPTIMIZER
    # ---------------------------------------------------------
    print("\nStarting Bayesian Search for Optimal Threshold...")
    print("-" * 50)
    
    # Define the boundaries of our search space
    pbounds = {'thresh': (0.02, 0.25)}
    ...
    # init_points: random steps to explore the space before starting Bayesian logic
    # n_iter: how many intelligent Bayesian optimization steps to perform
    optimizer.maximize(
        init_points=3, 
        n_iter=20, 
    )
    ...
    # Print Summary
    print("\n" + "="*50)
    print(" TUNING SUMMARY (Bayesian Search)")
    print("="*50)
    print(f"🏆 BEST MATCH: {optimizer.max}")
    print(f"Set your config.json threshold to: {optimizer.max['params']['thresh']:.4f}")
    print("="*50)
