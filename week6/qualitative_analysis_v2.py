...
#     output_dir = os.path.join("qualitative_results", f"SPOTTING_ANALYSIS_{args.model_best}")
...
#         # --- 1. PROBABILITY CURVES PLOT ---
#         plt.figure(figsize=(10, 5))
#         frames = np.arange(args.clip_len)
        
#         # Plot only relevant classes (those in GT or with high confidence)
#         relevant_classes = set(gt_classes) | {c+1 for c, p in enumerate(max_probs_best) if p > 0.15}
        
#         for c_idx in sorted(list(relevant_classes)):
#             c_name = idx_to_class[c_idx]
#             plt.plot(frames, probs_best_seq[:, c_idx], label=f"Best: {c_name}", linewidth=2.5)
#             plt.plot(frames, probs_base_seq[:, c_idx], label=f"Base: {c_name}", linestyle='--', alpha=0.5)

#         # Draw Vertical lines for Ground Truth labels
#         for lbl in clip_labels:
#             plt.axvline(x=lbl['label_idx'], color='red', linestyle=':', alpha=0.8)
#             plt.text(lbl['label_idx'], 0.95, 'GT', color='red', fontweight='bold', ha='center')

#         plt.title(f"Temporal Confidence Curves (Clip {idx})", fontweight='bold')
...
#         plt.tight_layout()
#         plt.savefig(os.path.join(output_dir, f'clip_{idx}_curves.png'))
#         plt.close()

#         # --- 2. DISTRIBUTION BAR CHART ---
#         plt.figure(figsize=(10, 5))
#         x_bars = np.arange(len(ordered_class_names))
#         plt.bar(x_bars - 0.2, max_probs_base, 0.4, label='Base Max', color='#A1C9F4', edgecolor='black')
#         plt.bar(x_bars + 0.2, max_probs_best, 0.4, label='Best Max', color='#FFB482', edgecolor='black')
#         plt.xticks(x_bars, ordered_class_names, rotation=45, ha='right')
#         plt.title(f"Max Confidence Comparison (Clip {idx})")
#         plt.legend()
#         plt.tight_layout()
...
#         # --- 3. VIDEO GENERATION ---
#         orig_h, width, _ = first_frame.shape
#         hud_h = 100
#         out_path = os.path.join(output_dir, f'clip_{idx}.mp4')
#         out = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc(*'mp4v'), playback_fps, (width, orig_h + hud_h))
        
#         print(f"Generating Clip {idx}")
#         for frame_idx, p in enumerate(actual_frame_paths):
#             frame = cv2.imread(p) if p and os.path.exists(p) else np.zeros((orig_h, width, 3), dtype=np.uint8)
#             canvas = np.zeros((orig_h + hud_h, width, 3), dtype=np.uint8)
#             canvas[hud_h:, :] = frame
            
#             p_base = get_top_preds(probs_base_seq[frame_idx], idx_to_class)
#             p_best = get_top_preds(probs_best_seq[frame_idx], idx_to_class)
            
#             cv2.putText(canvas, f"Frame {frame_idx}", (15, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
#             cv2.putText(canvas, f"Base: {p_base}", (15, 55), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 220, 255), 2)
#             cv2.putText(canvas, f"Best: {p_best}", (15, 85), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 220, 255), 2)
            
#             for lbl in clip_labels:
#                 if abs(lbl['label_idx'] - frame_idx) <= 2:
#                     cv2.putText(canvas, f"** {idx_to_class[lbl['label']]} **", (width//2-100, hud_h+50), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,255), 3)
#             out.write(canvas)
#         out.release()
