...
class ActionSpotDataset(Dataset):
    ...
    #ADDED MULTICLIP
    def get_clip_with_offset(self, idx, offset):
        """
        Re-extract a shifted clip from the same video as sample idx.

        offset is in sampled-frame units.
        Real frame shift = offset * self._stride.
        """
        base_paths = copy.deepcopy(self._frame_paths[idx])

        base_path = base_paths[0]
        start = base_paths[1]
        pad_start = base_paths[2]

        # Recover video name from base path
        video_name = os.path.basename(base_path)

        # Reconstruct nominal original clip start
        original_start = start - pad_start * self._stride

        shifted_start = original_start + offset * self._stride
        shifted_end = shifted_start + self._clip_len * self._stride

        shifted_paths = self._frame_reader.load_paths(
            video_name,
            shifted_start,
            shifted_end,
            stride=self._stride
        ...
        # fallback to original if shifted clip has no valid frames
        if shifted_paths[1] == -1:
            shifted_paths = base_paths
