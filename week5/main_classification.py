...
#Multiclip
from util.eval_multiclip_classification import evaluate_multiclip
...
def update_args(args, config):
    ...
    #STAGE 3
    args.temporal_head = config.get('temporal_head', 'max_pool')
    ...
    #STAGE 4 Multiclip
    args.multiclip_eval = config.get('multiclip_eval', False)
    args.multiclip_offsets = config.get('multiclip_offsets', [0])
    args.multiclip_aggregation = config.get('multiclip_aggregation', 'mean')
    args.checkpoint_path = config.get('checkpoint_path', None)

    #STAGE 5 Temp Shift (light)
...
def compute_sample_weights(train_data, num_classes):
    ...
    for i in range(len(train_data)):
        ...
        if len(pos_indices) > 0:
            ...
            weight = 1.0  # fallback (should rarely happen)
...
def main(args):
    ...
    if args.multiclip_eval:
        ap_score = evaluate_multiclip(
            model,
            test_data,
            offsets=args.multiclip_offsets,
            aggregation=args.multiclip_aggregation
        )
    else:
