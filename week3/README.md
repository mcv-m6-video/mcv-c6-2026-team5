...
* `generate_detections.py`: Script to extract and cache raw object detections (to speed up the hyperparameter search).
* `run_optical_flow_eval.py`: Evaluate the methods for calculating the optical flow (PyFlow and NeuFlow v2).
...
* **`optical_flow/`**: Contains optical flow estimation and evaluation methods.
* `flow_metrics.py`: Computes optical flow evaluation metrics.
* `infer_hf.py`: Inference utilities for HuggingFace optical flow models.
* `pyflow_estimator.py`: Wrapper for PyFlow optical flow estimation.
* `state_of_art_estimators.py`: Interface for state-of-the-art optical flow methods.
* `utils.py`: Utility functions for optical flow processing.
* `neuflow_v2/`: Implementation of NeuFlow v2 optical flow algorithm (code from original repository).


* **`pyflow/`**: PyFlow optical flow implementation (code from original repository).
...
* PyFlow optical flow implementation (code from original repository). Includes C++ source code and compiled binaries for fast optical flow computation.

### `/neuflow_v2` Directory

* NeuFlow v2 neural optical flow implementation (code from original repository). Contains the encoder-decoder architecture for learning-based optical flow estimation.
