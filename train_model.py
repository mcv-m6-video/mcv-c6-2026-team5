...
class EarlyBirdAggregation(nn.Module):
    """
    Concat + two Conv2D → 128 channels.
    Matches the aggregation described in EarlyBird §3.3 exactly.
    """

    def __init__(self, in_channels: int, n_cameras: int, out_channels: int = 128):
        super().__init__()
        self.reduce = nn.Sequential(
            nn.Conv2d(in_channels * n_cameras, out_channels,
    ...
    def forward(self, bev: torch.Tensor) -> torch.Tensor:
        ...
        return self.reduce(bev.reshape(B, S * C, Hg, Wg))   # (B, 128, Hg, Wg)
...
class EarlyBirdModel(nn.Module):
    ...
    def __init__(
        ...
        # ── BEV aggregation: concat + 2×Conv (paper §3.3) ─────────────
...
def build_targets(
    ...
    for cam_idx in range(n_cam):
        ...
        # Feature map spatial size matches encoder output: H/4 × W/4
...
def train(
    ...
    val_camera_configs:   list[dict] | None = None,  # None = skip validation
    n_identities:         int | None = None,           # auto-counted if None
    epochs:               int   = 50,
    batch_size:           int   = 2,
    grad_accum:           int   = 8,
    lr:                   float = 1e-3,
    ...
    val_every:            int   = 5,   # print full val metric summary every N epochs
